VXA.SpiceImport = (function() {
  ...
  function parse(text) {
    ...
    lines.forEach(function(line) {
      ...
      // Sprint 60: 8 new SPICE elements — S/B/E/F/G/H/K/T
        ...
        // Voltage-controlled switch: S1 n+ n- nc+ nc- modelname
        circuit.parts.push({ type: 'switch', nodes: [gn(tk[1]), gn(tk[2])], model: tk[5] || '', closed: true });
      }
      else if (ch === 'B') {
        // Behavioral source: B1 n+ n- V={expr} or I={expr}
        var bRest = line.substring(tk[0].length).trim();
        var bExprMatch = bRest.match(/(?:\S+\s+\S+\s+)?(?:V|I)\s*=\s*(.+)/i);
        if (!bExprMatch) bExprMatch = tk.slice(3).join(' ').match(/(?:V|I)\s*=\s*(.+)/i);
        var bExpr = bExprMatch ? bExprMatch[1].trim() : '0';
        circuit.parts.push({ type: 'behavioral', nodes: [gn(tk[1]), gn(tk[2])], expression: bExpr });
      }
      else if (ch === 'E') {
        // VCVS: E1 n+ n- nc+ nc- gain
        circuit.parts.push({ type: 'vcvs', nodes: [gn(tk[1]), gn(tk[2]), gn(tk[3]), gn(tk[4])], val: pv(tk[5] || '1') });
      }
      else if (ch === 'G') {
        // VCCS: G1 n+ n- nc+ nc- gm
        circuit.parts.push({ type: 'vccs', nodes: [gn(tk[1]), gn(tk[2]), gn(tk[3]), gn(tk[4])], val: pv(tk[5] || '0.001') });
      }
      else if (ch === 'F') {
        // CCCS: F1 n+ n- Vname gain
        circuit.parts.push({ type: 'cccs', nodes: [gn(tk[1]), gn(tk[2]), gn(tk[1]), gn(tk[2])], val: pv(tk[4] || '1') });
      }
      else if (ch === 'H') {
        // CCVS: H1 n+ n- Vname gain
        circuit.parts.push({ type: 'ccvs', nodes: [gn(tk[1]), gn(tk[2]), gn(tk[1]), gn(tk[2])], val: pv(tk[4] || '1000') });
      }
      else if (ch === 'K') {
        // Coupled inductors: K1 L1 L2 coupling
        circuit.parts.push({ type: 'coupled_l', nodes: [], val: pv(tk[3] || '1'), ref1: tk[1], ref2: tk[2] });
      }
      else if (ch === 'T') {
        // Transmission line: T1 n1+ n1- n2+ n2- Z0=50 TD=1n
  ...
  function placeCircuit(circuit) {
    ...
    // Wire each node's pins — direct pin-to-pin (chain topology)
    Object.keys(nodePositions).forEach(function(nodeIdx) {
      ...
      for (var i = 0; i < positions.length - 1; i++) {
        ...
        // Direct wire from pin A to pin B (buildCircuit uses coordinate key matching)
