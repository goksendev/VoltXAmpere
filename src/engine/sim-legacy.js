...
function buildCircuitFromCanvas() {
  ...
  let nodeCount = 0;
  // Sprint 57: pin registry — exact positions from part pins. Wire endpoints
  // snap to the nearest registered pin within SNAP_TOL px.
  const SNAP_TOL = 25;
  const _pinPositions = []; // [{x,y,idx}] — populated by registerPin only
  function registerPin(x, y) {
    var rx = Math.round(x), ry = Math.round(y);
    var key = rx + ',' + ry;
    if (!pinMap.has(key)) {
      var idx = nodeCount++;
      pinMap.set(key, idx);
      _pinPositions.push({ x: rx, y: ry, idx: idx });
    }
  ...
  function getNode(x, y) {
    ...
    // Sprint 57: snap to nearest registered pin within SNAP_TOL
    var bestDist = SNAP_TOL + 1, bestIdx = -1;
    for (var k = 0; k < _pinPositions.length; k++) {
      var pp = _pinPositions[k];
      var dist = Math.max(Math.abs(pp.x - rx), Math.abs(pp.y - ry));
      if (dist < bestDist) { bestDist = dist; bestIdx = pp.idx; }
    }
    if (bestIdx >= 0) return bestIdx;
    // No nearby pin → create new node (wire endpoint in open space)
    var idx2 = nodeCount++;
    pinMap.set(key, idx2);
    _pinPositions.push({ x: rx, y: ry, idx: idx2 });
    return idx2;
  }
  // register all part pins FIRST (exact positions)
  const partPinNodes = [];
  for (const p of S.parts) {
    const pins = getPartPins(p);
    partPinNodes.push(pins.map(pin => registerPin(pin.x, pin.y)));
  }
  // 2. Union-Find to merge wire-connected nodes
  // Wire endpoints now snap to nearest pin via getNode()
  const uf = new UnionFind(nodeCount + 500); // headroom for wire-only nodes
  for (const w of S.wires) {
    const n1 = getNode(w.x1, w.y1), n2 = getNode(w.x2, w.y2);
    uf.union(n1, n2);
  }
  // Sprint 57: proximity merge is now handled by getNode() snap-to-nearest-pin.
  // The O(n²) allKeys merge loop is retained with a TIGHT tolerance (2px) for
  // exact-overlap cases only — it must NOT use SNAP_TOL (25px) because that
  // would false-merge separate pins on closely-spaced components (e.g. op-amp
  // +/- at 30px apart). getNode() snap is one-directional: wire endpoint →
  // nearest registered pin. The 2px loop catches rounding dust.
